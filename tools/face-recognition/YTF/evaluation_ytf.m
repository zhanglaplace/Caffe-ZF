...
model   = '/data/youtubeface/face_deploy.prototxt';
...
%% get image list 
root_folder = '/data/youtubeface/aligned_images_DB-112X96';
image_list = get_image_list_in_folder(root_folder);
save ytf_eval_list.mat image_list;
%load ytf_eval_list.mat


subset_map = create_folder_map(image_list);
save ytf_subset_map.mat subset_map;
...
save('features_ASsoftmax.mat'.'features','-v7.5');
%load ../result/ytf/feature_assoftmax.mat
...
for cross = 1:10
    ...
    [~, accuracy, deci] = libsvmpredict(double(C{5}(C{1}==cross)),mean_distances(C{1}==cross),model);
%     cmd = [' -t 2 -h 0'];
