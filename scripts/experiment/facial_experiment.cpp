...
// 备注:
/*******************************************************/
cv::Mat findNonReflectiveTransform(std::vector<cv::Point2d> source_points, std::vector<cv::Point2d> target_points, cv::Mat Tinv = cv::Mat()) {
...
void phase_affect_datasets(){
	const string srcName = "/home/sdc1/zf/datasets/face-expression/affectnets/training.csv"; //原文件
	const string dstName  = "/home/sdc1/zf/datasets/face-expression/affectnets/affectNet_train-114X114.txt"; //保存的txt img,class_type face_x face_y face_x+face_w face_y+face_h
	const string tmpImg = "/home/zf/deeplearning/caffe/data/AffectNet/Manually_Annotated_Images-144X144-train/"; // 保存生成后的目录
	const string dirPath = "/home/sdc1/zf/datasets/face-expression/affectnets/Manually_Annotated_Images/"; //原始图片路径
	...
	while (getline(fin, lines)){
        ...
        if (access(tmp_folder.c_str(), 0) == -1){
            if(-1 == mkdir(tmp_folder.c_str(),S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH))
                cout<<"error\n";
        }
		...
		left_eye.x = left_eye.x/6;
        left_eye.y = left_eye.y/6;
        right_eye.x = right_eye.x/6;
		right_eye.y = right_eye.y/6;
		...
		// 写入txt并统计各个类别的数据
		fou << target_filename << " " << atoi(line[6].c_str())  << endl;
