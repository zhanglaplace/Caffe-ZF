...
#include "caffe/layers/angular_triplet_layer.hpp"

namespace caffe {

template <typename Dtype>
void AngularTripletLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  type_ = this->layer_param_.angular_triplet_param().type();
  iter_ = this->layer_param_.angular_triplet_param().iteration();
  lambda_ = (Dtype)0.;

  const int num_output = this->layer_param_.angular_triplet_param().num_output();
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.angular_triplet_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > weight_filler(GetFiller<Dtype>(
        this->layer_param_.angular_triplet_param().weight_filler()));
...
template <typename Dtype>
void AngularTripletLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  switch (type_) {
  case AngularTripletParameter_MarginType_SINGLE:
    break;
  case AngularTripletParameter_MarginType_DOUBLE:
    cos_theta_quadratic_.Reshape(top_shape);
    break;
  case AngularTripletParameter_MarginType_TRIPLE:
    cos_theta_quadratic_.Reshape(top_shape);
    cos_theta_cubic_.Reshape(top_shape);
    sign_1_.Reshape(top_shape);
    sign_2_.Reshape(top_shape);
    break;
  case AngularTripletParameter_MarginType_QUADRUPLE:
    cos_theta_quadratic_.Reshape(top_shape);
    cos_theta_cubic_.Reshape(top_shape);
...
template <typename Dtype>
void AngularTripletLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  iter_ += (Dtype)1.;
  Dtype base_ = this->layer_param_.angular_triplet_param().base();
  Dtype gamma_ = this->layer_param_.angular_triplet_param().gamma();
  Dtype power_ = this->layer_param_.angular_triplet_param().power();
  Dtype lambda_min_ = this->layer_param_.angular_triplet_param().lambda_min();
...
template <typename Dtype>
void AngularTripletLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  Dtype* top_data = top[0]->mutable_cpu_data();
  const bool triplet_flag = this->layer_param_.angular_triplet_param().triplet();  
  const bool semihard_flag = this->layer_param_.angular_triplet_param().semihard();
  const bool hard_flag = (type_ != AngularTripletParameter_MarginType_SINGLE); 
...
#ifdef CPU_ONLY
STUB_GPU(AngularTripletLayer);
#endif

INSTANTIATE_CLASS(AngularTripletLayer);
REGISTER_LAYER_CLASS(AngularTriplet);
