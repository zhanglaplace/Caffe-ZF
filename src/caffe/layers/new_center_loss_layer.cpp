...
#include "caffe/layers/new_center_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe{
	template<typename Dtype>
	void NewCenterLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		alpha = this->layer_param_.new_center_loss_param().alpha();
		lossWeight = this->layer_param_.new_center_loss_param().loss_weight();
		clusterNum = this->layer_param_.new_center_loss_param().cluster_num();
	...
	template<typename Dtype>
	void NewCenterLossLayer<Dtype>::Forward_cpu(
	...
	template<typename Dtype>
	void NewCenterLossLayer<Dtype>::Backward_cpu(
		const vector<Blob<Dtype>*> &top,
	...
	template <typename Dtype>
	void NewCenterLossLayer<Dtype>::Forward_gpu(
		const vector<Blob<Dtype>*>& bottom,
		const vector<Blob<Dtype>*>& top){
		Forward_cpu(bottom, top);
	}
	template<typename Dtype>
	void NewCenterLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom){
		Backward_cpu(top, propagate_down,bottom);
	}
	INSTANTIATE_CLASS(NewCenterLossLayer);
	REGISTER_LAYER_CLASS(NewCenterLoss);
}
