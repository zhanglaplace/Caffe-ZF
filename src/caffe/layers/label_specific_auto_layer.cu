...
#include "caffe/layers/label_specific_auto_layer.hpp"

namespace caffe {

  template <typename Dtype>
  __global__ void LabelSpecificAutoForward(const int n, const int dim, const Dtype* label,
  ...
  template <typename Dtype>
  void LabelSpecificAutoLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    ...
    LabelSpecificAutoForward<Dtype> << <CAFFE_GET_BLOCKS(num), CAFFE_CUDA_NUM_THREADS >> > (
      num, dim, label_data, top_data, bias_);
    CUDA_POST_KERNEL_CHECK;
  }

  template <typename Dtype>
  void LabelSpecificAutoLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  INSTANTIATE_LAYER_GPU_FUNCS(LabelSpecificAutoLayer);
}  // namespace caffe