...
__global__ void SoftmaxOHEMForwardGPU(const int nthreads,
...
template <typename Dtype>
void SoftmaxWithOHEMLayer<Dtype>::Forward_gpu(
  ...
  SoftmaxOHEMForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
...
template <typename Dtype>
__global__ void SoftmaxOHEMBackwardGPU(const int nthreads, const Dtype* top,
...
template <typename Dtype>
void SoftmaxWithOHEMLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    if (use_hard_mining_) {
      ...
      Dtype* counts = prob_.mutable_gpu_diff();
      // NOLINT_NEXT_LINE(whitespace/operators)
      SoftmaxOHEMBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
...
INSTANTIATE_LAYER_GPU_FUNCS(SoftmaxWithOHEMLayer);
